...
def load_data():  
    ...
    try:
        ...
        st.error("If running locally, adjust file paths accordingly or place the CSVs in the same directory as the script.")
        st.stop()
...
st.markdown("Explore various energy and development metrics across countries over time.")
    ...
    for i in range(len(fig.data)):
        ...
        step = dict(
            ...
            args=[{'visible': [False] * len(fig.data)},  # Set all traces to invisible
                  {'title.text': f'{column_name} Map - {year_label}'}], # Update title dynamically
            label=year_label
        )
...
if selected_column:
    ...
    if fig_streamlit:
        ...
        st.info("Select another metric or check data availability for the selected metric.")
